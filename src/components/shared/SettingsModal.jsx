  ...
  return (
    <div className="cat-row">
      ...
      <div className="cat-row__info">
        ...
        <div className="cat-row__colors">
          {cat.dot} · {cat.bg} · {cat.color}
        </div>
      </div>
      <button
        onClick={() => onEdit(cat)}
        className="btn-ghost"
        style={{ color: "var(--c-accent)", fontSize: 11 }}
      >
        {t("settings.edit")}
      </button>
      {cat.id !== "other" && (
        <button
          ...
          className="btn-ghost"
          style={{ color: "var(--c-missed)", fontSize: 11 }}
        >
          <X />
        </button>
      )}
  ...
  const field = ({ key, label, placeholder }) => (
    <div className="field" key={key}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        {(key === "dot" || key === "bg" || key === "color") && (
          <ColorDot color={form[key]} />
        )}
        <input
          value={form[key]}
    ...
    <div className="cat-form">
      <div className="cat-form__title">
        {isNew
          ...
          : t("settings.editCategory", { name: initial.label })}
      </div>
      {FIELDS.map(field)}
...
export default function SettingsModal({ onClose }) {
  ...
  const catList = Object.values(cats).sort(
  ...
  async function handleDelete(id) {
    if (!confirm(t("settings.deleteConfirm", { name: cats[id]?.label })))
      return;
              ...
              {calendars.map((cal) => (
                ...
                >
                  <input
                    ...
                    style={{
                      accentColor: cal.backgroundColor ?? "var(--c-teal)",
                      width: 16,
                      height: 16,
                    }}
                  />
                  <div
                    style={{
                      ...
                      height: 12,
                      borderRadius: "50%",
                      background: cal.backgroundColor ?? "var(--c-teal)",
                      flexShrink: 0,
                    }}
                  />
