...
export default function DayColumn({
  ...
  calEvents.forEach((e) => {
    ...
    if (linked) {
      ...
      const actual = Math.max(
        linked.actual_duration ?? linked.duration ?? 0,
        eventDur
      );
      planHours += planned;
  ...
  return (
    ...
    >
      <div
        className={`day-col__header${isToday ? " day-col__header--today" : ""}`}
      >
        {shortLabel}
        <button className="btn-icon-nav" onClick={() => setCreating(true)}>
          <Plus size={14} />
        </button>
        {isToday && (
          <span className="badge badge--today">{t("dayColumn.today")}</span>
        )}
      </div>
        ...
        <div className="day-col__calendar">
          {calEvents.map((e) => {
            const linkedTask = tasks.find(
              (task) => task.linked_event_id === e.id
            );
            const isLinking = linkingEventId === e.id;
            const unlinkableTasks = tasks.filter(
              (task) => !task.linked_event_id || task.linked_event_id === e.id
            );
            ...
            return (
              <div
                key={e.id}
                className={isCompleted ? "calendar-card__completed" : ""}
              >
                <div className="calendar-card__content">
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {!linkedTask &&
                        (isLinking ? (
                          <X
                            ...
                            className="calendar-icon"
                            onClick={() =>
                              setLinkingEventId(isLinking ? null : e.id)
                            }
                          />
                        ) : (
                          <Link2
                    ...
                    {e.time && <div className="task-card__dur">{e.time}</div>}
                    {linkedTask && (
                      <div className="calendar__linked-task">
                        <Link2 size={12} className="calendar-icon" />
                        <span className="calendar__linked-task-text">
                          {linkedTask.text}
                        </span>
                        <X
                          size={12}
                          className="calendar-icon"
                          onClick={() => handleUnlink(linkedTask.id)}
                        />
                      </div>
                    )}
                    {isLinking && (
                      <select
                        ...
                        defaultValue=""
                        onChange={(e2) =>
                          e2.target.value && handleLink(e2.target.value, e.id)
                        }
                      >
                  ...
                  {e.durationHours && (
                    <span className="task-card__dur">
                      {formatDuration(e.durationHours, locale)}
                    </span>
                  )}
                </div>
              </div>
      ...
      {(planLabel || factLabel) && (
        <div className="day-col__footer">
          {planLabel && (
            <span>
              {t("weekView.planPrefix")} {planLabel}
            </span>
          )}
          {planLabel && factLabel && <span> · </span>}
          {factLabel && (
            <span>
              {t("weekView.factPrefix")} {factLabel}
            </span>
          )}
        </div>
      )}
