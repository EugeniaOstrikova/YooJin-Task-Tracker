...
export function useTasks() {
  ...
  const toggleDone = useCallback(
    async (id) => {
      ...
      // ← новое: если выполнили и actual_duration не задан — берём duration
      const actual_duration =
        done && !task.actual_duration && task.duration
          ? task.duration
          : done
      ...
      setTasks((prev) =>
        prev.map((t) =>
          t.id === id ? { ...t, done, completed_at, actual_duration } : t
        )
      );
      await patchTask(id, { done, completed_at, actual_duration }).catch(
        console.error
      );
    },
    [tasks]
